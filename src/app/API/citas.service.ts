...
export class CitasService {
  ...
  public obtenerHorariosDisponibles(fecha: any): Observable<any> {
    return this.http
      .get<citas>(`${this.API}/citas/horarios-disponibles`, fecha)
      .pipe(
        catchError((error) => {
          console.log(error);
          return throwError(() => error);
  ...
  apartarCita(data: any): Observable<any> {
    ...
    return this.http.post(`${this.API}/citas/apartar`, data).pipe(
      catchError((error) => {
  ...
  obtenerCitasPorUsuario(userId: string): Observable<any[]> {
    return this.http.get<any[]>(`${this.API}/citas/usuario/${userId}`).pipe(
      catchError((error) => {
        console.log('error:', error);
        return throwError(() => error);
      })
